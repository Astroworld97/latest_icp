...
def create_random_quat():
    ...
    # return [0.707, 0, 0.707, 0] #90 deg rotation about x 
...
def flip_180(point_cloud, colorDict, height):
    quat = [0,0,1,0] #270 deg rotation about centroid
    norm = quat_norm(quat)
    quat = [quat[0]/norm, quat[1]/norm, quat[2]/norm, quat[3]/norm]
    quat_star = quat_conjugate(quat)
    p_centroid = point_cloud_centroid(point_cloud)
    ...
    for i in range(len(point_cloud)):
        point = point_cloud[i]
        color = colorDict[tuple(point)]
        # point = [point[0]-p_centroid[0], point[1]-p_centroid[1], point[2]-p_centroid[2]]
        colorDict[tuple(point)] = ()
        left = quat_mult(quat, point) 
        right = quat_mult(left, quat_star) 
        rotation = right 
        moved_point = [rotation[1] + translation_vector[0], rotation[2] + translation_vector[1], rotation[3] + translation_vector[2]]
        moved_point = [moved_point[0]+p_centroid[0], moved_point[1]+p_centroid[1], moved_point[2]+height]
        point_cloud[i] = moved_point
        colorDict[tuple(moved_point)] = color
    return point_cloud, colorDict 
...
def closest_point_on_cylinder(point, height, rad, origin, colorDictP, modelBlueRange, modelRedRange):
    ...
    if point[2]>=(height): #point lies above cylinder model in z   
        ...
        if color_match(point_color, (0.0, 1.0, 1.0)):
            ...
            z = height - 2.0
    elif (height - 2.0)<=point[2]<height: #point lies somewhere within blue range of cylinder model
        if color_match(point_color, (240/360, 1.0, 1.0)):
            z = point[2]
        else:
            z = 2.00
...
def generate_point_cloud_p(r, height, colorDict): #cylinder point cloud: r = radius; h = height
    ...
    for i in range(num_points):
        ...
        if random.choice([True, False]):
            ...
            z = random.uniform(height-2.0, height)
        point = [x,y,z]
        points.append(point)
    for point in points:
        if point[2] >= height-2.0:
            #blue hsv values
            hue = 240/360
            s = 1.0
            v = 1.0
...
def plot(point_cloud_p, colorDict):
    ...
    colorWood_rgb = colorsys.hsv_to_rgb(colorWood[0], colorWood[1], colorWood[2])
